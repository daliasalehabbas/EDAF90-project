...
export class GraphViewerComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if(this.duration == 'default'){
      this.coin = ['BTC']
      this.base = 'USD'
       await this.get_daily_rates(this.base, this.coin);
       this.get_24hour()
    }
    this.chart = new Chart('canvas', {
      ...
      data: {
        ...
        datasets: [{
          label: 'Rates between ' + this.base + ' and ' + this.coin[0],
  ...
  async set_duration(interval: string) {
    ...
    if (this.duration.toLowerCase() == 'week') {
      ...
      await this.get_historic_rates(this.base, this.coin[0])
    ...
    this.get_24hour();
    await this.get_daily_rates(this.base, this.coin[0]);
  }
